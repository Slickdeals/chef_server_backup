...
describe 'chef_server_backup::build_jobs' do
  context 'When all attributes are default, and sparsely create populated data bag' do
    let(:chef_run) do
      ChefSpec::ServerRunner.new(step_into: ['chef_server_backup_job']) do |node, server|
    ...
    it 'creates the knife.pem' do
      expect(chef_run).to create_template('/foo/bar/dc01/dc01-knife.pem')
    end

    it 'creates the knife.rb' do
      expect(chef_run).to create_template('/foo/bar/dc01/dc01-knife.rb')
    end
  ...
  context 'When all attributes are default, and sparsely populated delete data bag' do
    ...
    it 'deletes the knife.pem' do
      expect(chef_run).to delete_file(::File.join(dest_dir, 'dc01-knife.pem'))
    end

    it 'deletes the knife.rb' do
      expect(chef_run).to delete_file(::File.join(dest_dir, 'dc01-knife.rb'))
    end
